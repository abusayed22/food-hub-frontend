...
export function Navbar() {
  ...
  return (
    ...
    >
      <div className="container flex items-center justify-between h-20 px-6 mx-auto md:px-12">
        {/* Logo */}
        <Link
          href="/"
          className={cn(
            "text-3xl font-bold tracking-widest font-serif",
            goldColorClass
          )}
        >
          ÉPICURE
        ...
        <nav className="hidden md:flex items-center gap-12">
          {navItems.map((item, index) => (
            <Link
              ...
              href={item.href}
              className={cn(
                "text-sm font-medium tracking-[0.2em] uppercase transition-colors hover:text-white",
                goldColorClass
              )}
            >
              {item.title}
        ...
        <div className="hidden md:flex items-center">
          <Button
            variant="ghost"
            className={cn(
              "text-sm font-medium tracking-[0.2em] uppercase hover:bg-transparent hover:text-white",
              goldColorClass
            )}
          >
            SIGN IN
