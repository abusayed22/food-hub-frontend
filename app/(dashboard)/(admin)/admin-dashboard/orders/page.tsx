...
const params = await searchParams;
    ...
    const { data, error } = await orderService.adminFetchOrder({ page, limit: 10, search }, { cache: 'no-store' }, API)
  ...
  return (
    <div className="min-h-screen bg-zinc-950 p-6 md:p-12 text-zinc-100">
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
               <div className="p-2 bg-[#C0A975]/10 rounded-lg">
                  <ShoppingBag className="h-6 w-6 text-[#C0A975]" />
               </div>
      ...
      <AdminOrderTable data={orders} meta={meta}/>
    </div>
    </div>
  )
}
